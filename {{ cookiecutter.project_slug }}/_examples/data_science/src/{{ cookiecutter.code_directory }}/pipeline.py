...
def run_inference(strategy_name, input_path=DEFAULT_INPUT, output_dir=DEFAULT_OUTPUT_DIR, params=None) -> Path:
    """Load inputs, run a strategy, save outputs and metadata.

    Args:
        strategy_name: Name of the InferenceStrategy subclass to use.
        input_path: Path to the input data.
        output_dir: Base output directory.  A timestamped subdirectory will
            be created under ``output_dir / strategy_name``.
    ...
    run_output_dir = output_dir / strategy_name / timestamp
    run_output_dir.mkdir(parents=True, exist_ok=True)

    # Save outputs
    save_outputs(outputs, run_output_dir)
    logger.info("Saved %d outputs to %s", len(outputs), run_output_dir)
...
def run_evaluation(evaluator_name, output_dir, expected_path) -> Path:
    """Run an evaluator, saving results into the same directory as outputs.

    Writes ``evaluation.json`` and any plots directly into *output_dir*
    (no nested subdirectory).

    Args:
        evaluator_name: Name of the AbstractEvaluator subclass to use.
        output_dir: Path to the inference run directory (contains outputs).
    ...
    predicted = load_outputs(output_dir)
    ...
    for name, fig in plots.items():
        ...
        logger.info("Saved plot %s.png to %s", name, output_dir)

    return output_dir


def run_pipeline(strategy_name, evaluator_name, input_path=DEFAULT_INPUT,
                 *, expected_path, output_dir=DEFAULT_OUTPUT_DIR, params=None) -> Path:
    ...
    Returns:
        The path to the run output directory.
    """
    run_output_dir = run_inference(strategy_name, input_path, output_dir, params=params)
    run_evaluation(evaluator_name, run_output_dir, expected_path)
    return run_output_dir
